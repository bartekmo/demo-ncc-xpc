...
variable subnets {
  ...
  validation {
    condition     = length(var.subnets) == 7
    error_message = "Please provide exactly 7 subnet names (external, internal, heartbeat, management)."
  }
}
...
variable frontends {
  type        = map(string)
  default     = {}
  description = "Map of public IP name->address to be linked as ELB frontend."
}
...
variable flexvm_tokens {
  type = list(string)
  description = "List of FlexVM tokens to apply to FGTs"
  default = []
}
...
variable cidrs_hq {
  type = map(string)
  description = "list of names and CIDRs in HQ to route via transit network [6]"
}

variable vpn_peers {
  type = list(string)
  description = "List of IP addresses for branch office VPN peers"
}

variable vpn_secret {
  type = string
  description = "Secret for branch VPNs"
}
